...
public class MatchingManager : Singleton<MatchingManager>
{
    ...
    private void SpawnBonus(Cell cell, int matchedCellCount)
    {
        ...
        {
            case >= 4:
                ItemType bonusItemType = (Random.value > 0.5f) ? ItemType.HorizontalRocket : ItemType.VerticalRocket;
                cell.item = ItemFactory.Instance.CreateItem(bonusItemType, board.itemsParent);
                break;
